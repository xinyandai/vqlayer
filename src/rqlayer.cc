...
RQLayer::RQLayer(RQLayer&& c) noexcept: I_(c.I_), O_(c.O_),
                                        type_(c.type_), code_(c.code_),
                                        dict_(c.dict_), norm_(c.norm_) {
  c.dict_ = nullptr;
  c.code_ = nullptr;
  c.norm_ = nullptr;
}
...
SparseVector RQLayer::backward(const SparseVector& g,
